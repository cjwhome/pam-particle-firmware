...
uint8_t SPS30::ReadFromSerial()
{
    ...
    {
        Serial.printf("CRC error. expected 0x%02X, got 0x%02X\n",_Receive_BUF[_Receive_BUF_Length-1], ret);
        ...
        if (_serial->available())
        {
            ...
            if (i == 0) {
                ...
                // check last byte received
                    ...
                    {
                        Serial.println("board cannot handle 115k so getting uncontrolled input");
                        return(SPS30_ERR_PROTOCOL);
                    }
