...
void SPS30::DebugPrintf(const char *pcFmt, ...)
{
    ...
    if (_SPS30_Debug > level){
        ...
        Serial.printf(prfbuf, pcFmt, pArgs);
        va_end(pArgs);
...
uint8_t SPS30::GetValues(struct sps_values *v)
{
    ...
    if ( !_started ) 
    {
        if ( ! start() )
        {
            Serial.println("Wasn't even able to start");
...

    {
        ...
        // fill buffer to send
        if (SHDLC_fill_buffer(SER_READ_MEASURED_VALUE) != true)
        {
            Serial.println("SER_READ_MEASURES_VALUE issue");
            return(SPS30_ERR_PARAMETER);
        }
        ...
        if (ret != SPS30_ERR_OK)
        {
            Serial.println("ReadFromSerial didn't work properly");
            return (ret);
        } 
...
bool SPS30::setSerialSpeed()
{
    ...
    Serial.printf("Using Hardware Serial port 4 for sps30\n\r");
    _Sensor_Comms = COMM_TYPE_SERIAL;
    // Serial4.begin(_Serial_baud, SERIAL_8N1, Serial_RX, Serial_TX);
...
uint8_t SPS30::SendToSerial()
{
    ...
    {
        Serial.println("SPS30_ERR_DATALENGTH");
        return(SPS30_ERR_DATALENGTH);
    }
    ...
    Serial.println("SendToSerial finished properly");
    return(SPS30_ERR_OK);
}
...
uint8_t SPS30::ReadFromSerial()
{
    ...
    if (ret != SPS30_ERR_OK)
    {
        Serial.println("SerialToBuffer failed!");
        Serial.println(ret);
        return(ret);
    }
