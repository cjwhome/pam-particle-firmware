...
nanoMY_pb_root = os.path.join(os.getcwd(), '..')
...
nanoMY_pb_protos = env.GetProjectOption("custom_nanoMY_pb_protos", "")
nanoMY_pb_plugin_options = env.GetProjectOption("custom_nanoMY_pb_options", "")

if not nanoMY_pb_protos:
    print("[nanopb] No generation needed.")
else:
    if isinstance(nanoMY_pb_plugin_options, (list, tuple)):
        nanoMY_pb_plugin_options = " ".join(nanoMY_pb_plugin_options)

    nanoMY_pb_plugin_options = nanoMY_pb_plugin_options.split()

    protos_files = fs.match_src_files(project_dir, nanoMY_pb_protos)
    if not len(protos_files):
        print("[nanopb] ERROR: No files matched pattern:")
        print(f"custom_nanoMY_pb_protos: {nanoMY_pb_protos}")
        exit(1)

    protoc_generator = os.path.join(nanoMY_pb_root, 'generator', 'protoc')

    nanoMY_pb_options = ""
    nanoMY_pb_options += f" --nanoMY_pb_out={generated_src_dir}"
    for opt in nanoMY_pb_plugin_options:
        nanoMY_pb_options += (" --nanoMY_pb_opt=" + opt)
    ...
    for proto_include_dir in proto_include_dirs:
        nanoMY_pb_options += (" --proto_path=" + proto_include_dir)
        nanoMY_pb_options += (" --nanoMY_pb_opt=-I" + proto_include_dir)
    ...
    for proto_file in protos_files:
        ...
        if not need_generate:
            ...
            cmd = protoc_generator + " " + nanoMY_pb_options + " " + proto_file_basename
