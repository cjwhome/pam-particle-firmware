...
#ifndef __PAMSERIALMANAGER_H__
#define __PAMSERIALMANAGER_H__
...
#define SERIAL_BUFF_LENGTH (1024)

class PAMSerialManager {

public:
    PAMSerialManager(Stream &_serial);
    ~PAMSerialManager();
    ...
    size_t println(uint16_t rd, const char c[]) {
        if (this->active_responders.size() == 0 || rd == this->active_responders.top()) {
            return this->serial->println(c);
        } else {
            return 0;
        }
    }

    uint16_t registerResponder(PAMSerialResponder *responder);
    // uint16_t registerResponder(pam_serial_responder_t *responder);

    void popResponder();
    void pushResponder(uint16_t responder);
...
private:
    ...
    std::vector<PAMSerialResponder *> responders;
    // std::vector<pam_serial_responder_t *> responders;
    std::stack<uint16_t> active_responders;
    // std::stack<pam_serial_responder_t *> active_responders;

    uint8_t data[SERIAL_BUFF_LENGTH];
    uint8_t data_index = 0;
    void clear_data();
};

#define PAMSerial __fetch_pam_usb_serial()
extern PAMSerialManager& __fetch_pam_usb_serial();

#endif // __PAMSERIALMANAGER_H__