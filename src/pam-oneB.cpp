...
int sendSerialMessage(String information);
void goToSleep(void);
...
#line 22 "c:/Users/abailly/OLD_PAM_PROJECTS/pam-particle-firmware/src/pam-oneB.ino"
PRODUCT_ID(15083);
PRODUCT_VERSION(10);
...
void serialMenu()
{
    ...
    byte systemStatus;
    incomingByte = '0';
    time_t timeout = Time.now();
    Serial.print("Menu>");
    Serial.flush();
    while (incomingByte != 'x' || timeout+15 >= Time.now())
    {
        if (serBuf.available()) 
        {
            String receivedData = serBuf.readStringUntil('\n');
